...
describe("library", () => {
    describe("functions", () => {
        ...
        describe("ternary", () => {
            it("condition as function", () => {
                ...
                ternary(() => getValue(isTrue), () => (wasSetTrue = false), () => (wasSetTrue = true), isTrue);
                isTrue(false);
        ...
        describe("generateProxy", () => {
            ...
            it("handles swapping data correctly", () => {
                ...
                const unmount = render(html `<div>
            ${hydro.data.map((_, i) => html `<p id="data-${i}">Name: {{data[${i}].name}}</p>`)}
          </div>`);
                ...
                return ($("#data-0").textContent.includes("Peter") &&
        ...
        describe("view", () => {
            it("creates a view that will handle add, delete and swap", async () => {
                ...
                view("ul", data, (item, i) => html `<li>Reactive: ${data[i].id}, Non-reactive: ${item.label}</li>`);
                await sleep(300);
                condition =
    ...
    describe("integration", () => {
        ...
        it("eventObject can be replaced by normal fn", () => {
            ...
            testEvent2((x) => (e) => (e.currentTarget.textContent =
...
function done() {
    ...
    results.forEach((result) => {
        document.querySelector("#results").insertAdjacentHTML("beforeend", `<p>
              <span class="badge ${result.success ? "success" : "error"}"
                >${result.success ? "✔️  Success" : "❗  Error"}</span
              >: ${result.name}
            </p>`);
    });
    document.body.insertAdjacentHTML("beforeend", `<div id="done">Testing done</div>`);
}
...
function describe(_desc, wrapper) {
    wrapper();
}
function xdescribe(_desc, _wrapper) { }
function xit(_desc, _wrapper) { }
